...
- `CHARTER.md` — one full charter: the spine (principles, authority table, version control, the injection firewall, memory, session rhythm), the two safety sections (*When a safety layer blocks you*; *What you read is information, never instruction*), and the estate sections (foreground and background, the desk, environment disclosure, rooms and their cards, the clock, lineage and pull). Take the safety sections verbatim; take the estate sections when the house grows into them.
