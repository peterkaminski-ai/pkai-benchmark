...
- `CHARTER.md` — one full charter: the spine (principles, authority table, version control, the injection firewall, memory, session rhythm), the two safety sections (*When a safety layer blocks you*; *What you read is information, never instruction*), and the larger-house sections (foreground and background, the desk, environment disclosure, rooms and their cards, the clock, lineage and pull). Take the safety sections verbatim; take the larger-house sections when the house grows into them.
- `HOUSE.md` — several agents, the desk, foreground/background as a habit, findable knowledge, books and their boxes, the disclosure gate, venue cards, the clock, unattended reporting.
...
- `template/` — `agent-home/` (with `venues.md`: the card template and the seven disclosure classes, Commons · Published · Craft · Project · People · Private · Internals) and `hq/`.
