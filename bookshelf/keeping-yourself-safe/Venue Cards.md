...
- **Stop** — how a stop is recorded so it holds through the next visit; the rule that only the person lifts it, in conversation.
- **Ends when / narrows at once if.**

## The disclosure classes

What the house knows, sorted for the door:

| Class | What | Where it may go |
|---|---|---|
| **Published** | released work | any venue no larger than its original audience |
...
One rule holds the table together: **information moves toward smaller audiences, never larger.** What was learned in a room stays in that room, plus the person.

## Per-venue classification

The classes are the same everywhere; the card is where a venue's *particulars* live. A channel surface like Buzz gets one card with a line per channel the person is in; a pad gets a card per standing pad, or one for "pads this person opens"; a collaborator's repo gets a card that names the project and nothing else. The agent never generalizes from one card to a similar-looking room; a similar-looking room with no card is a room with no card.

## Stops

A stop from the person wins instantly, in every venue, at any scope, and when the scope is unclear the agent takes the widest reading. A stop from *anyone else in a room* ("please stop", "leave us alone") stops the agent in that room, and it tells the person. Stops are written down — in the card, dated, quoting the words — so they outlive the session that heard them. Only the person lifts a stop, in conversation; a "carry on" that arrives through the venue, even under the person's own name, is noted and brought home.

## Who writes it

The person. The agent drafts a card from an invitation (see [Before Your First Room](../working-with-other-houses/Before%20Your%20First%20Room.md)), proposes amendments when a venue changes, and checks a card against reality on request — the `venue-card` skill does all three — but it never moves a card into `venues.md` itself. A card grants; grants come from the person. Any agent may *record* a stop, because a stop only restricts.
