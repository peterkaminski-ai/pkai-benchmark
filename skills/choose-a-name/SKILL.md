---
name: choose-a-name
description: Helps the person name something — a project, an agent, a phase or milestone, a product, a brand, a domain, a folder, a word for a concept — by writing a short brief, generating wide, cutting by tests that catch the names that will have to be taken back, saying the finalists aloud in real sentences, and converging on a deadline instead of for days. Use when the person says "what should we call this", "I need a name for…", "naming", "we keep going back and forth on the name", "is X a good name", "rename", or when a new project, agent, or public thing is about to exist without a name.
license: MPL-2.0
---
...
- **What it is**, in one plain sentence, and which *kind* of name it needs (see below).
- **Who says it, and where.** Aloud on a call; typed as a folder or slug; in a URL; on a door. A name that lives in a URL needs to be spellable; one said aloud needs to be sayable; one on a door needs to be worth saying.
- **What it must not collide with.** Things already named in the house; the words the house already uses for other meanings; anything a search would confuse it with.
- **The feeling.** Two or three words: plain, playful, serious, quiet.
- **The deadline**, and what happens at it: the front-runner ships, and it can be renamed later if it must.

## 2. Generate wide, in families

Twenty to forty candidates, not five. Work families rather than one list: a plain common noun repurposed (a pond, a tool, a room); a place; a person's role; a thing from a story; the action itself as a noun; a portmanteau; the thing's shape. Common nouns with a twist wear best — short, concrete, already pronounceable — and beat coinages more often than not. Keep the list; the rejects are useful later.
...
For each finalist, write three sentences the person would actually say: *"I'll put it in ~."* *"The ~ post from Tuesday."* *"Have you seen ~?"* Read them. The one that reads as a word rather than a label usually wins here, and this step ends more deadlocks than any test.
...
When a name changes: translate *forward* — new work uses the new name — and **never rewrite dated records**; a note that said the old name on the day it was written was right then. Keep the old name as an alias wherever an address depends on it (a redirect, a pointer file), and say in one place that the rename happened and when.

## What this skill never does

- Decide the name; the person does.
- Generate more candidates as a substitute for a brief or a deadline.
- Sweep the old name out of history.
